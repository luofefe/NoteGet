...
@class_case_log
class restoreRecycleBinInput(unittest.TestCase):
    envData = read_env_yaml()  # 小驼峰  工具类对象的实例化、环境变量、【base】
    apiData = read_api_yaml()['restoreRecycleBin']
    ...
    def testCase01_input_must_key(self, dic):
        ...
        body.pop(dic['key'])
        res = self.ar.patch(url=url, body=body, sid=self.sid, user_id=self.userId)
    ...
    def testCase03_input(self):
        """【恢复回收站】noteIds 存在子对象 [234, [123, 123]]"""
        step('请求恢复回收站接口')
        url = f'{self.host}{self.apiData["path"]}'
        note_id = str(int(time.time() * 1000)) + '_noteId'
        # body = self.reBase
