...
@class_case_log
class updateNoteSubjectInput(unittest.TestCase):
    envData = read_env_yaml()  # 小驼峰  工具类对象的实例化、环境变量、【base】
    apiData = read_api_yaml()['updateNoteSubject']
    ...
    def testCase02_input(self):
        """【上传/更新便签信息主体】noteId 英文的大小写"""
        step('请求上传/更新便签信息主体接口')
        url = f'{self.host}{self.apiData["path"]}'
        note_id = str(int(time.time() * 1000)) + '_noteId'
        # body = self.reBase
        body = {
